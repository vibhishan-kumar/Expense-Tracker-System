...
import { ArrowLeft, Download, BarChart2, Users, BookOpen, Home, IndianRupee, TrendingUp, TrendingDown, Info, Calendar, Activity, ChevronDown, ChevronUp } from 'lucide-react';
...
const HOSTEL_COLORS = { NRS: '#7C3AED', MH: '#2563EB', LH: '#D97706' };
const BAR_COLOR = '#7C3AED';
const PEER_BAR = '#22C55E';
// ─── Tooltip for Recharts ──────────────────────────────────────
const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        return (<div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-sm)', padding: '10px 14px', fontSize: '0.85rem' }}>
        <p style={{ color: 'var(--text-primary)', fontWeight: 600, marginBottom: 4 }}>{label}</p>
        {payload.map((p) => (<p key={p.dataKey} style={{ color: p.color, margin: '2px 0' }}>
...
// ─── Insight Badge ─────────────────────────────────────────────
const InsightBadge = ({ text, type = 'info' }) => {
    const colors = {
    ...
    const c = colors[type];
    return (<div style={{ background: c.bg, color: c.text, borderRadius: 'var(--radius-sm)', padding: '10px 14px', fontSize: '0.88rem', display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '10px' }}>
      <Info size={15} style={{ flexShrink: 0, marginTop: 2 }}/>
      <span>{text}</span>
    </div>);
};
// ─── Tab Button ────────────────────────────────────────────────
const TabBtn = ({ icon, label, active, onClick }) => (<button onClick={onClick} style={{
...
const Reports = () => {
    ...
    // Data states
    const [catData, setCatData] = useState({ expenses: [], incomes: [] });
    ...
    const exportPDF = () => {
        ...
        if (activeTab === 'category') {
            ...
            const grandTotalExp = expenses.reduce((s, c) => s + Number(c.total), 0);
            autoTable(doc, {
                startY: 32,
            ...
            if (incomes.length > 0) {
                const grandTotalInc = incomes.reduce((s, c) => s + Number(c.total), 0);
                doc.text("Income Breakdown", 14, doc.lastAutoTable.finalY + 15);
                autoTable(doc, {
            ...
            autoTable(doc, {
                ...
                body: semData.semester_peers.map((p) => [
                    p.name,
            ...
            if (semData.sem_category_avg && semData.sem_category_avg.length > 0) {
                ...
                autoTable(doc, {
                    ...
                    body: semData.sem_category_avg.map((r) => [r.category, `₹${Number(r.sem_avg).toFixed(0)}`]),
                });
            }
            ...
            autoTable(doc, {
                ...
                body: hostelData.hostel_stats.map((h) => [
    ...
    const exportExcel = () => {
        ...
        if (activeTab === 'category') {
            ...
            const incomes = catData.incomes || [];
            const grandTotalExp = expenses.reduce((s, c) => s + Number(c.total), 0);
            const wsExp = XLSX.utils.json_to_sheet(expenses.map((c) => ({
                Category: c.category,
            ...
            if (incomes.length > 0) {
                const grandTotalInc = incomes.reduce((s, c) => s + Number(c.total), 0);
                const wsInc = XLSX.utils.json_to_sheet(incomes.map((c) => ({
                    Category: c.category,
            ...
            const ws1 = XLSX.utils.json_to_sheet(semData.semester_peers.map((p) => ({
                Name: p.name,
            ...
            if (semData.sem_category_avg?.length > 0) {
                const ws2 = XLSX.utils.json_to_sheet(semData.sem_category_avg.map((r) => ({
                    Category: r.category,
            ...
            const ws = XLSX.utils.json_to_sheet(hostelData.hostel_stats.map((h) => ({
            ...
            const ws = XLSX.utils.json_to_sheet(balanceTrend.map((d) => ({
                Date: d.label,
                'Income (₹)': Number(d.income).toFixed(0),
                'Expense (₹)': Number(d.expense).toFixed(0),
    ...
    const buildTypeComparisonData = () => {
        if (!typeData)
            return [];
        const myMap = {};
        typeData.my_spending.forEach((r) => { myMap[r.category] = Number(r.my_total); });
        const peerMap = {};
        typeData.peer_avg.forEach((r) => { peerMap[r.category] = Number(r.peer_avg); });
    ...
    const buildSemPeerData = () => {
        if (!semData)
            return [];
        return semData.semester_peers.map((p) => ({
            name: p.name.split(' ')[0],
    ...
    const buildHostelData = () => {
        if (!hostelData || hostelData.not_applicable)
            return [];
        return hostelData.hostel_stats.map((h) => ({
            hostel: h.hostel_name,
            'Avg Expense': Number(h.avg_expense).toFixed(0),
            'Max Expense': Number(h.max_expense).toFixed(0),
            students: h.student_count,
    ...
    const getTypeInsights = () => {
        ...
        if (insights.length === 1)
            insights.push({ text: 'Your spending is in line with the UoH average for your peer group!', type: 'good' });
        return insights;
    };
    const getSemInsights = () => {
        if (!semData)
            return [];
        const peers = semData.semester_peers;
        const me = peers.find((p) => p.id === userId);
        if (!me)
            return [{ text: `Comparing with Semester ${semData.user.semester} peers.`, type: 'info' }];
        const allExpenses = peers.map((p) => Number(p.total_expense)).filter((v) => v > 0);
        const avg = allExpenses.reduce((a, b) => a + b, 0) / (allExpenses.length || 1);
    ...
    // ═══════════════════ RENDER ════════════════════════════════
    return (<div className="container animate-fade-in" style={{ padding: '40px 24px', maxWidth: '1100px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <button onClick={() => setShowTabs(!showTabs)} className="glass-panel" style={{ width: '100%', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-secondary)', border: '1px solid var(--glass-border)', cursor: 'pointer', color: 'var(--text-primary)', fontWeight: 600, transition: 'all 0.2s' }}>
        ...
        {showTabs && (<div className="animate-fade-in" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '12px', padding: '16px', background: 'rgba(255,255,255,0.02)', borderRadius: 'var(--radius-md)', border: '1px solid var(--glass-border)' }}>
      ...
      {loading && (<div className="glass-panel" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)' }}>
          Loading UoH analytics...
        </div>)}

      {/* ── Export Buttons (always visible) ────────────────── */}
      {!loading && (<div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginBottom: '4px' }}>
      ...
      {!loading && activeTab === 'category' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>

          {(!catData.expenses || catData.expenses.length === 0) && (!catData.incomes || catData.incomes.length === 0) ? (<div className="glass-panel" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)' }}>
              ...
              {catData.expenses && catData.expenses.length > 0 && (<>
                  <div className="glass-panel animate-fade-in" style={{ padding: '24px' }}>
                    ...
                    <ResponsiveContainer width="100%" height={260}>
                      <PieChart>
                        <Pie data={(catData.expenses || []).map((c) => ({ ...c, total: Number(c.total) }))} dataKey="total" nameKey="category" cx="50%" cy="50%" outerRadius={100} label={(props) => `${props.name} (${((props.percent || 0) * 100).toFixed(0)}%)`} labelLine={true}>
                          {(catData.expenses || []).map((c, i) => <Cell key={i} fill={c.color || `hsl(${i * 37}, 70%, 55%)`}/>)}
                        </Pie>
                    ...
                    <div className="table-container">
                      ...
                      <thead>
                        <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                          {['Category', 'Total Spent', 'Transactions', '% of Expenses'].map(h => (<th key={h} style={{ textAlign: 'left', padding: '10px 12px', color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600 }}>{h}</th>))}
                        </tr>
                      </thead>
                      <tbody>
                        {(() => {
                        const expenses = catData.expenses || [];
                        const grandTotal = expenses.reduce((s, c) => s + Number(c.total), 0);
                        return expenses.map((c, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--glass-border)' }}>
              ...
              {catData.incomes && catData.incomes.length > 0 && (<>
                  <div className="glass-panel animate-fade-in" style={{ padding: '24px' }}>
                    ...
                    <ResponsiveContainer width="100%" height={260}>
                      <PieChart>
                        <Pie data={(catData.incomes || []).map((c) => ({ ...c, total: Number(c.total) }))} dataKey="total" nameKey="category" cx="50%" cy="50%" outerRadius={100} label={(props) => `${props.name} (${((props.percent || 0) * 100).toFixed(0)}%)`} labelLine={true}>
                          {(catData.incomes || []).map((c, i) => <Cell key={i} fill={c.color || `hsl(${120 + i * 37}, 70%, 50%)`}/>)}
                        </Pie>
                        <Tooltip formatter={(v) => `₹${Number(v).toFixed(2)}`}/>
                      </PieChart>
                    </ResponsiveContainer>
                  ...
                  <div className="glass-panel animate-fade-in" style={{ padding: '24px' }}>
                    ...
                    <div className="table-container">
                      ...
                      <thead>
                        <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                          {['Category', 'Total Received', 'Transactions', '% of Income'].map(h => (<th key={h} style={{ textAlign: 'left', padding: '10px 12px', color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600 }}>{h}</th>))}
                        </tr>
                      </thead>
                      <tbody>
                        {(() => {
                        const incomes = catData.incomes || [];
                        const grandTotal = incomes.reduce((s, c) => s + Number(c.total), 0);
                        return incomes.map((c, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--glass-border)' }}>
                              <td style={{ padding: '12px 12px', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      {!loading && activeTab === 'type' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {!typeData ? (<div className="glass-panel" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)' }}>No data.</div>) : (<>
              {/* Info Header */}
              ...
              {buildTypeComparisonData().length > 0 && (<div className="glass-panel" style={{ padding: '24px' }}>
              ...
              <div className="glass-panel" style={{ padding: '24px' }}>
                ...
                <div className="table-container">
                  ...
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                      {['Category', 'My Spending', 'Peer Avg', 'Difference', 'Status'].map(h => (<th key={h} style={{ textAlign: 'left', padding: '10px 12px', color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600 }}>{h}</th>))}
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    const isOver = diff > 0;
                    return (<tr key={i} style={{ borderBottom: '1px solid var(--glass-border)' }}>
                          ...
                          <td style={{ padding: '12px' }}>
                            {row['Peer Average'] === 0 ? (<span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>No peer data</span>) : isOver ? (<span style={{ color: 'var(--danger)', display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.82rem' }}><TrendingUp size={13}/> Over Average</span>) : (<span style={{ color: 'var(--success)', display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.82rem' }}><TrendingDown size={13}/> Under Average</span>)}
                          </td>
                        </tr>);
                })}
                  </tbody>
      ...
      {/* ── TAB 3: vs Semester ───────────────────────────────────── */}
      {!loading && activeTab === 'semester' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {!semData ? (<div className="glass-panel" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)' }}>No data.</div>) : (<>
              {/* Info header */}
              <div className="glass-panel" style={{ padding: '20px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              {semData.sem_category_avg && semData.sem_category_avg.length > 0 && (<div className="glass-panel" style={{ padding: '24px' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                        {['Category', 'Peer Avg Spend', 'Bar'].map(h => (<th key={h} style={{ textAlign: 'left', padding: '10px 12px', color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600 }}>{h}</th>))}
                      </tr>
                    </thead>
                    <tbody>
                      {(() => {
                        const max = Math.max(...semData.sem_category_avg.map((r) => Number(r.sem_avg)));
                        return semData.sem_category_avg.map((r, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--glass-border)' }}>
      ...
      {!loading && activeTab === 'hostel' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {!hostelData ? (<div className="glass-panel" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)' }}>Loading...</div>) : hostelData.not_applicable ? (<div className="glass-panel" style={{ padding: '40px', textAlign: 'center' }}>
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={buildHostelData()} margin={{ top: 0, right: 10, left: 0, bottom: 20 }}>
                    ...
                    <Bar dataKey="Avg Expense" radius={[4, 4, 0, 0]}>
                      {buildHostelData().map((h, i) => (<Cell key={i} fill={HOSTEL_COLORS[h.hostel] || BAR_COLOR}/>))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="glass-panel" style={{ padding: '24px' }}>
                ...
                <div className="table-container">
                  ...
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                      {['Hostel', 'Students', 'Avg Expense', 'Min Expense', 'Max Expense'].map(h => (<th key={h} style={{ textAlign: 'left', padding: '10px 12px', color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600 }}>{h}</th>))}
                    </tr>
                  </thead>
                  <tbody>
                    {hostelData.hostel_stats.map((h, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--glass-border)', background: h.hostel_name === myHostelName ? 'rgba(124,58,237,0.07)' : 'transparent' }}>
              ...
              {hostelData.hostel_category_breakdown && hostelData.hostel_category_breakdown.length > 0 && (<div className="glass-panel" style={{ padding: '24px' }}>
                  <h3 style={{ color: 'var(--text-primary)', marginBottom: '20px', fontSize: '1rem' }}>Category Spending — Avg per Hostel</h3>
                  {(() => {
                        // Group by hostel
                        const grouped = {};
                        hostelData.hostel_category_breakdown.forEach((r) => {
                            if (!grouped[r.hostel_name])
                                grouped[r.hostel_name] = [];
                            grouped[r.hostel_name].push(r);
                        });
                        // Build recharts-friendly data
                        const categories = Array.from(new Set(hostelData.hostel_category_breakdown.map((r) => r.category)));
                        const hostels = Object.keys(grouped);
                        const chartData = categories.map(cat => {
                            const row = { category: cat };
                            hostels.forEach(h => {
                                const match = grouped[h].find((r) => r.category === cat);
                        ...
                        <BarChart data={chartData} margin={{ top: 0, right: 10, left: 0, bottom: 40 }}>
                          ...
                          <Tooltip content={<CustomTooltip />}/>
                          <Legend wrapperStyle={{ color: 'var(--text-secondary)', paddingTop: '12px' }}/>
                          {hostels.map(h => (<Bar key={h} dataKey={h} name={`${h} Avg`} fill={HOSTEL_COLORS[h] || BAR_COLOR} radius={[4, 4, 0, 0]}/>))}
                        </BarChart>
                      </ResponsiveContainer>);
                    })()}
                </div>)}
      ...
      {!loading && activeTab === 'time' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {/* Sub-toggle */}
          <div style={{ display: 'flex', gap: '10px' }}>
            <button onClick={() => setTimeView('monthly')} className="btn-primary" style={{ background: timeView === 'monthly' ? 'var(--accent-gradient)' : 'var(--bg-secondary)', fontSize: '0.85rem', padding: '7px 16px' }}>Monthly (last 6 months)</button>
            <button onClick={() => setTimeView('weekly')} className="btn-primary" style={{ background: timeView === 'weekly' ? 'var(--accent-gradient)' : 'var(--bg-secondary)', fontSize: '0.85rem', padding: '7px 16px' }}>Weekly (last 12 weeks)</button>
          </div>

          {!timeSummary ? (<div className="glass-panel" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)' }}>No data available.</div>) : (() => {
                const data = timeView === 'monthly' ? timeSummary.monthly : timeSummary.weekly;
                ...
                const bestPeriod = [...data].sort((a, b) => a.expense - b.expense)[0];
                const worstPeriod = [...data].sort((a, b) => b.expense - a.expense)[0];
                return (<>
                {/* Summary cards */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px' }}>
                  {[
                    ...
                    ].map(c => (<div key={c.label} className="glass-panel" style={{ padding: '18px', background: c.bg }}>
                ...
                <div className="glass-panel" style={{ padding: '24px', overflowX: 'auto' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.88rem' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                        {['Period', 'Income (₹)', 'Expense (₹)', 'Net (₹)'].map(h => <th key={h} style={{ textAlign: 'left', padding: '8px 12px', color: 'var(--text-secondary)' }}>{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {data.map((d, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--glass-border)', opacity: 0.9 }}>
      ...
      {/* ── TAB 6: Balance Trend ──────────────────────────────────── */}
      {!loading && activeTab === 'trend' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {balanceTrend.length === 0 ? (<div className="glass-panel" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)' }}>No transaction data to show trend.</div>) : (() => {
                ...
                <div className="glass-panel" style={{ padding: '24px' }}>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={balanceTrend} margin={{ top: 5, right: 30, bottom: 30, left: 10 }}>
                      ...
                      <Line type="monotone" dataKey="balance" name="Balance" stroke="#7C3AED" strokeWidth={2.5} dot={false} activeDot={{ r: 5, stroke: '#7C3AED', fill: '#fff' }}/>
                    </LineChart>
                  </ResponsiveContainer>
                </div>
                ...
                <div className="glass-panel" style={{ padding: '24px', overflowX: 'auto' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.88rem' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                        {['Date', 'Income', 'Expense', 'Running Balance'].map(h => <th key={h} style={{ textAlign: 'left', padding: '8px 12px', color: 'var(--text-secondary)' }}>{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {[...balanceTrend].reverse().slice(0, 15).map((d, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--glass-border)' }}>
                          <td style={{ padding: '10px 12px', color: 'var(--text-primary)', fontWeight: 500 }}>{d.label}</td>
                          <td style={{ padding: '10px 12px', color: 'var(--success)' }}>+₹{d.income.toFixed(0)}</td>
                          <td style={{ padding: '10px 12px', color: 'var(--danger)' }}>-₹{d.expense.toFixed(0)}</td>
